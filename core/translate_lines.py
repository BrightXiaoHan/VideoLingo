...
def translate_lines(lines, previous_content_prompt, after_cotent_prompt, things_to_note_prompt, summary_prompt, index = 0):
    ...
    def retry_translation(prompt, length, step_name):
        ...
        raise ValueError(f'[red]❌ {step_name.capitalize()} translation of block {index} failed after 3 retries. Please check `{get_output_dir()}/gpt_log/error.json` for more details.[/red]')
    ...
    if len(lines.split('\n')) != len(translate_result.split('\n')):
        console.print(Panel(f'[red]❌ Translation of block {index} failed, Length Mismatch, Please check `{get_output_dir()}/gpt_log/translate_expressiveness.json`[/red]'))
